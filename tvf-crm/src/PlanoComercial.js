  ...
  return (
    ...
    <div className="tela-relatorio">
      ...
      <div className="kanban-toolbar" style={{ marginBottom: 16 }}>
        <label style={{ fontSize: 12, color: 'var(--text-3)' }}>Mês
          <input type="month" className="lm-input" style={{ marginLeft: 8 }} value={mesReferencia} onChange={e => setMesReferencia(e.target.value)} />
        </label>
        <span style={{ fontSize: 12, color: 'var(--text-3)' }}>Dias úteis: {duTotais} total, {duRestantes} restante(s)</span>
        ...
        <label style={{ fontSize: 12, color: 'var(--text-3)', marginLeft: 'auto' }}>Exportar
        ...
        <div className="lm-resumo" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {staff.filter(s => s.perfil === 'Supervisor' || s.perfil === 'Gestor').map(s => (
              <label key={s.id} style={{ fontSize: 13, color: 'var(--text-1)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <input type="checkbox" checked={s.plano_comercial_ativo !== false} onChange={e => alternarAtivo(s.id, e.target.checked)} />
                {s.nome} <span style={{ color: 'var(--text-3)', fontSize: 11 }}>({s.perfil})</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="lm-resumo" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            {ORDEM_VERTICAIS.map(v => (
              <label key={v} style={{ fontSize: 12, color: 'var(--text-2)', display: 'flex', flexDirection: 'column', gap: 4 }}>
