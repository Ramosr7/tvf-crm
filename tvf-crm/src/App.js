...
function LeadCard({ lead, expanded, onToggle, onMover, onFechar, obsTexto, onObsChange, onSalvarObs }) {
  ...
  // Classificar leads
  const leadsNovos = leads.filter(l => !l.acionar_especialista_historico && l.etapa_followup <= 1)
  const leadsRecontatos = leads.filter(l => l.etapa_followup > 1 && l.status !== 'fechado')
  const leadsFechados = leads.filter(l => l.status === 'fechado')
